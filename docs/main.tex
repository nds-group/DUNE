...
\begin{equation}
\max_{\pmb{x}} \; \frac{1}{c(\pmb{x})}\cdot\pmb{g}^T\pmb{x} \qquad \text{s.t.} \quad \pmb{Ax}=\pmb{1},\; \pmb{1}_n^T\pmb{x} \leq k,
...
The parameter $k \in \{2, \dots, m\}$ is an optional upper bound on the number of blocks in the partition, reflecting a deployment constraint on the number of available programmable devices; when unspecified (\ie $k=m$), the constraint is trivially satisfied for any partition and the problem reduces to the unconstrained formulation.
...
\State $n^* \gets m$, $\textit{gain-max} \gets 0$, $(\textit{blocks}) \gets \text{all single-element blocks}$
\While{$|\textit{blocks}| > 1$} \label{alg:for-m}
    \State $\textit{tuples} \gets \Call {combinations}{\textit{blocks}, 2}$
    \ForAll{$ \textit{tuple} \in \textit{tuples}$} \label{alg:for-gains-begin}
        \State $\textit{gains} \gets \Call {computeBlockGain}{\textit{tuple}}$
    \EndFor \label{alg:for-gains-end}
    \State $\textit{best tuple} \gets argmax(\textit{gains})$ \label{alg:gain-max}
    \ForAll{$\textit{block} \in \textit{blocks}$} \label{alg:for-update-begin}
    ...
    \If{$k = \emptyset$ \textbf{or} $|\textit{blocks}| \leq k$} \label{alg:store-max-begin}
        \State $\textit{gain} \gets \Call{computePartitionGain}{blocks}$
        \If{$\textit{gain}>\textit{gain-max}$}
            \State $n^* \gets |\textit{blocks}|$
...
%
When the partition is feasible (\ie $k=\emptyset$ or $|\textit{blocks}|\leq k$), its total gain is computed using~\eqref{eq:opt-objective} and possibly stored as the best solution (lines~\ref{alg:store-max-begin}--\ref{alg:store-max-end}).
When $k$ is specified, the gain-maximization is restricted to partitions with at most $k$ blocks, thus enforcing the cardinality constraint in~\eqref{eq:opt-objective}.
...
\input{score_table_CR}

\input{resource_usage_table_CR}

\input{total_latency_table}
